...
def _extract_developer_token(index_js_page):
    ...
    for token in token_matches:
        ...
        if (
            header_data.get("kid") == APPLE_MUSIC_DEVELOPER_TOKEN_KEY_ID
            or payload_data.get("iss") == APPLE_MUSIC_DEVELOPER_TOKEN_ISSUER
        ):
            return token
...
def apple_music_add_account(media_user_token):
    ...
    new_user = {
        ...
        "service": "apple_music",
        "active": True,
        "login": {"media-user-token": media_user_token},
    }
        ...
        account_pool.append(
            {
                ...
                if account_data.get("meta", {}).get("subscription", {}).get("active")
                else "free",
                "bitrate": "256k",
...
def apple_music_get_search_results(session, search_term, content_types):
    ...
    results = make_call(
        f"{BASE_URL}/catalog/{session.cookies.get('itua')}/search",
        params=params,
        session=session,
        skip_cache=True,
    )
                ...
                search_results.append(
                    {
                        ...
                        "item_thumbnail_url": track.get("attributes", {})
                        .get("artwork", {})
                        .get("url")
                        .replace("{w}", "160")
                        .replace("{h}", "160"),
                ...
                search_results.append(
                    {
                        ...
                        "item_thumbnail_url": album.get("attributes", {})
                        .get("artwork", {})
                        .get("url")
                        .replace("{w}", "160")
                        .replace("{h}", "160"),
                ...
                search_results.append(
                    {
                        ...
                        "item_thumbnail_url": artist.get("attributes", {})
                        .get("artwork", {})
                        .get("url")
                        .replace("{w}", "160")
                        .replace("{h}", "160"),
                ...
                search_results.append(
                    {
                        ...
                        "item_thumbnail_url": playlist.get("attributes", {})
                        .get("artwork", {})
                        .get("url")
                        .replace("{w}", "160")
                        .replace("{h}", "160"),
...
def apple_music_get_track_metadata(session, item_id):
    ...
    track_data = make_call(
        f"{BASE_URL}/catalog/{session.cookies.get('itua')}/songs/{item_id}",
        params=params,
        session=session,
    )
    try:
...
def apple_music_get_artist_album_ids(session, artist_id):
    ...
    album_data = make_call(
        f"{BASE_URL}/catalog/{session.cookies.get('itua')}/artists/{artist_id}",
        params=params,
        session=session,
    )
...
def apple_music_get_playlist_data(session, playlist_id):
    ...
    playlist_data = make_call(
        f"{BASE_URL}/catalog/{session.cookies.get('itua')}/playlists/{playlist_id}",
        session=session,
        skip_cache=True,
    )
