...
from ..api.soundcloud import (
    ...
    soundcloud_get_token,
    soundcloud_get_track_metadata,
)
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, _dialog, start_url="", parsingworker=None):
        ...
        if parsingworker:
            parsingworker.error.connect(
                self.show_popup_dialog, Qt.ConnectionType.QueuedConnection
            )

        for i in range(config.get("maximum_queue_workers")):
            queueworker = QueueWorker()
            queueworker.add_item_to_download_list.connect(
                self.add_item_to_download_list
            )
            queueworker.error.connect(
                self.show_popup_dialog, Qt.ConnectionType.QueuedConnection
            )
            queueworker.start()
    ...
    def get_icon(self, name):
        if name not in self.icon_cache:
            icon_path = os.path.join(
                config.app_root, "resources", "icons", f"{name}.png"
            )
    ...
    def bind_button_inputs(self):
        ...
        self.btn_progress_retry_all.clicked.connect(
            self.retry_cancelled_and_failed_downloads
        )
        self.btn_progress_cancel_all.clicked.connect(self.cancel_all_downloads)
        self.btn_audio_download_path_browse.clicked.connect(
            lambda: self.select_dir(self.audio_download_path)
        )
        self.btn_video_download_path.clicked.connect(
            lambda: self.select_dir(self.video_download_path)
        )

        self.btn_download_tmp_browse.clicked.connect(
            lambda: self.select_dir(self.tmp_dl_root)
        )
        self.tmp_dl_root.textChanged.connect(self.update_tmp_dir)
        self.search_term.returnPressed.connect(self.fill_search_table)
        self.btn_progress_clear_complete.clicked.connect(
            self.remove_completed_from_download_list
        )

        self.btn_search_filter_toggle.clicked.connect(
            lambda toggle: (
                self.group_search_items.show()
                if self.group_search_items.isHidden()
                else self.group_search_items.hide()
            )
        )
        self.btn_search_filter_toggle.clicked.connect(
            lambda switch: (
                ...
                if self.group_search_items.isHidden()
                else self.btn_search_filter_toggle.setIcon(self.get_icon("collapse_up"))
            )
        )
        self.btn_download_filter_toggle.clicked.connect(
            lambda toggle: (
                self.group_download_items.show()
                if self.group_download_items.isHidden()
                else self.group_download_items.hide()
            )
        )
        self.btn_download_filter_toggle.clicked.connect(
            lambda switch: (
                ...
                if self.group_download_items.isHidden()
                else self.btn_download_filter_toggle.setIcon(
                    self.get_icon("collapse_down")
                )
            )
        )

        self.download_queue_show_waiting.stateChanged.connect(
            self.update_table_visibility
        )
        self.download_queue_show_failed.stateChanged.connect(
            self.update_table_visibility
        )
        self.download_queue_show_unavailable.stateChanged.connect(
            self.update_table_visibility
        )
        self.download_queue_show_cancelled.stateChanged.connect(
            self.update_table_visibility
        )
        self.download_queue_show_completed.stateChanged.connect(
            self.update_table_visibility
        )
        ...
        self.mirror_spotify_playback.stateChanged.connect(
            self.manage_mirror_spotify_playback
        )

        self.settings_bookmark_accounts.clicked.connect(
            lambda: self.settings_scroll_area.verticalScrollBar().setValue(0)
        )
        self.settings_bookmark_general.clicked.connect(
            lambda: self.settings_scroll_area.verticalScrollBar().setValue(328)
        )
        self.settings_bookmark_audio_downloads.clicked.connect(
            lambda: self.settings_scroll_area.verticalScrollBar().setValue(1176)
        )
        self.settings_bookmark_audio_metadata.clicked.connect(
            lambda: self.settings_scroll_area.verticalScrollBar().setValue(2019)
        )
        self.settings_bookmark_video_downloads.clicked.connect(
        ...
        self.export_logs.clicked.connect(
            lambda: (
                shutil.copy(
                    os.path.join(
                        cache_dir(), "logs", config.session_uuid, "onthespot.log"
                    ),
                    os.path.join(os.path.expanduser("~"), "Downloads", "onthespot.log"),
                )
                and self.show_popup_dialog(
                    self.tr("Logs exported to '{0}'").format(
                        os.path.join(
                            os.path.expanduser("~"), "Downloads", "onthespot.log"
                        )
                        or True
                    )
                )
        ...
        self.donate.clicked.connect(
            lambda: open_item("https://justin025.github.io/about.html")
        )
    ...
    def set_table_props(self):
        ...
        self.tbl_search_results.horizontalHeader().setSectionResizeMode(
        ...
        self.tbl_dl_progress.horizontalHeader().setSectionResizeMode(
    ...
    def setup_spotify_oauth_override(self):
        ...
        if hasattr(self, "gb2"):
            ...
            if gb2_index is not None:
                ...
                spotify_oauth_override_gb = QGroupBox(
                    self.tr("Spotify Web API Override Credentials")
                )
                spotify_oauth_override_gb.setStyleSheet(
                    "QGroupBox { font-weight: bold; }"
                )
                spotify_oauth_override_layout = QVBoxLayout()

                override_explanation = QLabel(
                    self.tr(
                ...
                override_explanation.setStyleSheet(
                    "color: #666; margin-bottom: 10px; font-weight: normal;"
                )
                ...
                self.spotify_webapi_override_client_id.setPlaceholderText(
                    self.tr("Leave empty to use default librespot authentication")
                )
                ...
                client_secret_layout.addWidget(
                    self.spotify_webapi_override_client_secret
                )
    ...
    def setup_metadata_toggles(self):
        ...
        if hasattr(self, "gb2"):
            ...
            if gb2_index is not None:
                # Create the Metadata Options group box
                metadata_options_gb = QGroupBox(
                    self.tr("Spotify Metadata Fetching Options")
                )
                ...
                self.fetch_artist_metadata = QCheckBox(
                    self.tr("Fetch Full Artist Metadata")
                )
                self.fetch_artist_metadata.setChecked(
                    config.get("fetch_genre_metadata", True)
                )
                self.fetch_artist_metadata.setLayoutDirection(
                    Qt.LayoutDirection.LeftToRight
                )
                metadata_options_layout.addWidget(self.fetch_artist_metadata)

                artist_desc = QLabel(
                    self.tr("Retrieves genre data. Adds 1 API call per track.")
                )
                artist_desc.setStyleSheet(
                    "color: #666; font-size: 10pt; margin-left: 17px; margin-bottom: 5px;"
                )
                artist_desc.setWordWrap(True)
                metadata_options_layout.addWidget(artist_desc)

                self.fetch_album_metadata = QCheckBox(
                    self.tr("Fetch Full Album Metadata")
                )
                self.fetch_album_metadata.setChecked(
                    config.get("fetch_extended_album_metadata", True)
                )
                self.fetch_album_metadata.setLayoutDirection(
                    Qt.LayoutDirection.LeftToRight
                )
                ...
                album_desc = QLabel(
                    self.tr(
                        "Retrieves label & copyright information. Adds 1 API call per track."
                    )
                )
                album_desc.setStyleSheet(
                    "color: #666; font-size: 10pt; margin-left: 17px; margin-bottom: 5px;"
                )
                ...
                self.fetch_audio_metadata.setChecked(
                    config.get("fetch_audio_features", True)
                )
                self.fetch_audio_metadata.setLayoutDirection(
                    Qt.LayoutDirection.LeftToRight
                )
                ...
                audio_desc = QLabel(
                    self.tr(
                        "Retrieves BPM, key, danceability. Adds 1 API call per track."
                    )
                )
                ...
                self.fetch_credits_metadata = QCheckBox(
                    self.tr("Fetch Credits Metadata")
                )
                self.fetch_credits_metadata.setChecked(
                    config.get("fetch_track_credits", True)
                )
                self.fetch_credits_metadata.setLayoutDirection(
                    Qt.LayoutDirection.LeftToRight
                )
                metadata_options_layout.addWidget(self.fetch_credits_metadata)

                credits_desc = QLabel(
                    self.tr("Retrieves producers & writers. Adds 1 API call per track.")
                )
                credits_desc.setStyleSheet(
                    "color: #666; font-size: 10pt; margin-left: 17px; margin-bottom: 5px;"
                )
    ...
    def language_change(self):
        ...
        self.show_popup_dialog(
            self.tr(
                "The application's language has been changed, please restart the app."
            )
        )
    ...
    def reset_app_config(self):
        ...
        self.show_popup_dialog(
            self.tr(
                "The application setting was cleared successfully !\n Please restart the application."
            )
        )
    ...
    def select_dir(self, output):
        ...
        path = QFileDialog.getExistingDirectory(
            self, "OnTheSpot - Select Directory", os.path.expanduser("~")
        )
        if path.strip() != "":
    ...
    def session_load_done(self):
        ...
        if config.get("check_for_updates"):
            if not is_latest_release():
                self.show_popup_dialog(
                    self.tr(
                        "<p>An update is available at the link below,<p><a style='color: #6495ed;' href='https://github.com/justin025/onthespot/releases/latest'>https://github.com/justin025/onthespot/releases/latest</a>"
                    )
                )
    ...
    def fill_account_table(self):
        ...
        for account in account_pool:
            ...
            radiobutton.clicked.connect(
                lambda: config.set(
                    "active_account_number", self.tbl_sessions.currentRow()
                )
            )
            ...
            service = QTableWidgetItem(
                str(account["service"]).replace("_", " ").title()
            )
            ...
            self.tbl_sessions.setItem(
                rows, 1, QTableWidgetItem(account["username"][:22])
            )
            self.tbl_sessions.setItem(rows, 2, QTableWidgetItem(service))
            self.tbl_sessions.setItem(
                rows, 3, QTableWidgetItem(str(account["account_type"]).title())
            )
    ...
    def add_item_to_download_list(self, item, item_metadata):
        ...
        actions = DownloadActionsButtons(
            item["local_id"],
            item_metadata,
            pbar,
            copy_btn,
    ...
    def update_item_in_download_list(self, item, status, progress):
        self.statistics.setText(
            self.tr("{0} / {1}").format(
                config.get("total_downloaded_items"),
                format_bytes(config.get("total_downloaded_data")),
            )
        )
        with download_queue_lock:
            ...
            while check_row < self.tbl_dl_progress.rowCount():
                ...
                if local_id in download_queue:
                    if download_queue[local_id]["item_status"] in (
                        ...
                        logger.debug(
                            f"Removing Row: {check_row} and mediaid: {local_id}"
                        )
            ...
            while row_count > 0:
                for local_id in download_queue.keys():
                    ...
                    if download_queue[local_id]["item_status"] == "Waiting":
                        ...
                        download_queue[local_id]["gui"]["status_label"].setText(
                            self.tr("Cancelled")
                        )
                        download_queue[local_id]["gui"]["status_label"].setText(
                            self.tr("Cancelled")
    ...
    def user_table_remove_click(self):
        ...
        if config.get("active_account_number") == index or config.get(
    ...
    def set_login_fields(self):
        ...
        if self.login_service.currentIndex() == 1:
            ...
            self.btn_login_add.clicked.connect(
                lambda: (
                    (
                        self.show_popup_dialog(
                            self.tr("Account added, please restart the app.")
                        )
                        or True
                    )
                    and apple_music_add_account(self.login_password.text())
                    and self.login_password.clear()
                )
            )
            ...
            self.btn_login_add.clicked.connect(
                lambda: (
                    (
                        self.show_popup_dialog(
                            self.tr(
                                "Public account added, please restart the app.\nLogging into personal accounts is currently unsupported, if you have any premium purchases please consider lending it to the dev team."
                            )
                        )
                        or True
                    )
                    and bandcamp_add_account()
                )
            )
        ...
        # Deezer
            ...
            self.btn_login_add.clicked.connect(
                lambda: (
                    ...
                    and deezer_add_account(self.login_password.text())
                    and self.login_password.clear()
                )
            )
        ...
        # Soundcloud
            ...
            self.btn_login_add.clicked.connect(
                lambda: (
                    ...
                    and soundcloud_add_account(oauth_token=self.login_password.text())
                    and self.login_password.clear()
                )
            )
        ...
        # Youtube Music
            ...
            self.btn_login_add.clicked.connect(
                lambda: (
                    (
                        self.show_popup_dialog(
                            self.tr("Public account added, please restart the app.")
                        )
                        or True
                    )
                    and youtube_music_add_account()
                )
            )
        ...
        # Generic (yt-dlp)
            ...
            self.lb_generic_extractors.setText(
                self.tr(
                    "<strong>The following services are officially supported by the Generic Downloader. Even if your website is not officially supported, generic downloader may be able to download media off it anyway.</strong><br>{0}"
                ).format("<br>".join(generic_list_extractors()))
            )
            self.login_username_label.hide()
            self.login_username.hide()
            self.login_password_label.hide()
            ...
            self.btn_login_add.clicked.connect(
                lambda: (
                    (
                        self.show_popup_dialog(
                            self.tr("Generic Downloader added, please restart the app.")
                        )
                        or True
                    )
                    and generic_add_account()
                )
            )
    ...
    def add_tidal_account(self):
        ...
        login_worker = threading.Thread(
            target=self.add_tidal_account_worker, args=(device_code,)
        )
        login_worker.daemon = True
        login_worker.start()
    ...
    def fill_search_table(self):
        ...
        if results is None:
            ...
            results is False
            and account_pool[config.get("active_account_number")]["service"]
            == "generic"
        ):
            self.show_popup_dialog(
                self.tr(
            ...
            self.show_popup_dialog(
                self.tr("Invalid item, please check your query or account settings")
            )
            self.search_term.setText("")
            return
        ...
        def download_btn_clicked(item_name, item_url, item_service, item_type, item_id):
            ...
            self.show_popup_dialog(
                self.tr(
                    "{0} is being parsed and will be added to the download queue shortly."
                ).format(f"{item_type.title()}: {item_name}"),
                download=True,
            )
        ...
        def copy_btn_clicked(item_url):
            ...
            self.show_popup_dialog(
                self.tr("The URL has been copied to the clipboard."), download=True
            )
        ...
        for result in results:
            ...
            download_btn.clicked.connect(
                lambda x, item_name=result["item_name"], item_url=result["item_url"], item_type=result["item_type"], item_id=result["item_id"], item_service=result["item_service"]: (
                    download_btn_clicked(
                        item_name, item_url, item_service, item_type, item_id
                    )
                )
            )
            ...
            copy_btn.clicked.connect(
                lambda x, item_url=result["item_url"]: copy_btn_clicked(item_url)
            )
            ...
            btn_widget.setStyleSheet(
                "QWidget { background-color: transparent !important; }"
            )
            ...
            if config.get("show_search_thumbnails"):
                ...
                item_label = LabelWithThumb(
                    result["item_name"], result["item_thumbnail_url"]
                )
            else:
                item_label = QLabel(self.tbl_dl_progress)
            ...
            self.tbl_search_results.setItem(
                rows, 1, QTableWidgetItem(str(result["item_by"]))
            )
            self.tbl_search_results.setItem(
                rows,
                2,
                QTableWidgetItem(result["item_type"].replace("podcast_", "").title()),
            )
    ...
    def update_table_visibility(self):
        ...
        for row in range(self.tbl_dl_progress.rowCount()):
            ...
            if label:
                ...
                if (
                    (status == self.tr("Waiting") and not show_waiting)
                    or (status == self.tr("Failed") and not show_failed)
                    or (status == self.tr("Unavailable") and not show_unavailable)
                    or (status == self.tr("Cancelled") and not show_cancelled)
                    or (status == self.tr("Already Exists") and not show_completed)
                    or (status == self.tr("Downloaded") and not show_completed)
                ):
                    self.tbl_dl_progress.hideRow(row)  # Hide the row
                else:
                    self.tbl_dl_progress.showRow(
                        row
                    )  # Show the row if the status is allowed
